...
/**
 * This is the model class for table "recide".
 *
 * The followings are the available columns in table 'recide':
...
class Recide extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('RES_USU_correl, RES_HOG_correl', 'required'),
			array('RES_USU_correl, RES_HOG_correl', 'length', 'max'=>10),
			array('RES_tipo', 'length', 'max'=>13),
			array('RES_inicio, RES_fin', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('RES_correl, RES_USU_correl, RES_HOG_correl, RES_inicio, RES_fin, RES_tipo', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('RES_tipo',$this->RES_tipo,true);
	 ...
	 * @return Recide the static model class
	 */
